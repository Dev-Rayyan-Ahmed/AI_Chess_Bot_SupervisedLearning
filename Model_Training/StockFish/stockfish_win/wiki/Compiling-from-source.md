...
branch='master'
github_user='official-stockfish_win'
...
make strip
mv stockfish_win.exe ../../stockfish_${arch_cpu}.exe
make clean
cd
```
</details>
...
```bash
$ mkdir -p ~/repos/official-stockfish_win
$ cd ~/repos/official-stockfish_win
...
```bash
$ file stockfish_win
stockfish_win: ELF 64-bit LSB shared object, ARM aarch64, version 1 (SYSV), dynamically linked, interpreter /system/bin/linker64, with debug_info, not stripped
```
...
aarch64-linux-android-strip stockfish_win
```
...
```bash
$ file stockfish_win
stockfish_win: ELF 64-bit LSB shared object, ARM aarch64, version 1 (SYSV), dynamically linked, interpreter /system/bin/linker64, stripped
```

It is a good idea to rename the binary:

```bash
$ mv stockfish_win stockfish_DEV_armv8
```
...
mv stockfish_win.exe ../../stockfish_win-x64${1}.exe
...
make strip COMP=mingw
mv stockfish_win.exe ../../stockfish_win-x64${1}-pgo.exe
make clean COMP=mingw
}
...
qemu-aarch64 stockfish_win compiler
qemu-aarch64 stockfish_win bench
...
# test: qemu's magic at work
qemu-arm stockfish_win compiler
qemu-arm stockfish_win bench
...
cd Stockfish/src
make -j build ARCH=armv8 COMP=clang CXX="zig c++ -target aarch64-linux-musl"
mv stockfish_win.exe stockfish_armv8
```
</details>
